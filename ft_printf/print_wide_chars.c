...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/01/19 20:02:27 by byoung-w          #+#    #+#             */
/*   Updated: 2015/01/31 15:33:58 by byoung-w         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t		count_wide_chars(t_conv *conv
										, wchar_t *string, size_t *length)
{
	...
	while (i < *length)
	{
		...
		if (conv->precision_set
			&& (total_bytes + add) > conv->precision)
			break ;
		total_bytes += add;
		i++;
	}
										...
										, size_t *written)
{
	...
	total_bytes = count_wide_chars(conv, string, &length);
	if (!conv->flags.left_justify && conv->width > total_bytes)
	{
		place_padding((conv->flags.pad_with_zeros ? '0' : ' ')
						, conv->width - total_bytes, written);
	}
	i = 0;
	while (i < length)
	{
	...
	if (conv->flags.left_justify && conv->width > total_bytes)
		place_padding(' ', conv->width - total_bytes, written);
}
